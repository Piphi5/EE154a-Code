/*****************************************************************
  ...
  employee) at the local, and you've found our code hlllllllllllllllllllllllaelpful,
...
void printAccel();
void printMag();
void printAttitude(float ax, float ay, float az, float mx, float my, float mz, float temp, float press, float hum, float gas, float dt, float battTemp, float currentDraw);
long t = 0;
long prev_t = 0;
void setup() {
    Serial.begin(9600);
    ...
    Wire.begin();
    if (imu.begin(107U, 30U, Wire) == false)  // with no arguments, this uses default addresses (AG:0x6B, M:0x1E) and i2c port (Wire).
    {
        Serial.println("Failed to communicate with LSM9DS1.");
        Serial.println("Double-check wiring.");
        Serial.println(
            "Default settings in this sketch will "
    ...
    SPI.begin(18, 19, 23, chipSelect_sd);
    ...
    File SDout = SD.open(filename, FILE_WRITE);
    SDout.println("time, pitch, roll, heading");
    SDout.close();
    t = micros();
    prev_t = t;
}
...
void loop() {
    ...
    float battTemp = 1 / (10000 + 3977 * log(analogRead(34) / 1023.0)) + 25;
    float battCurrent = analogRead(13) / 1023.0 / 10.0;
    float dt = (float)(t - prev_t) / 1e6;
    printAttitude(imu.ax, imu.ay, imu.az,
                  -imu.my, -imu.mx, imu.mz, temp, pressure, humidity, gas, dt, battTemp, battCurrent);
...
void printAttitude(float ax, float ay, float az, float mx, float my, float mz, float temp, float press, float hum, float gas, float dt, float battTemp, float currentDraw) {
    ...
    File SDout = SD.open(filename, FILE_APPEND);
    Serial.print(millis());
    SDout.print(millis());
    SDout.print(", ");
    ...
    SDout.print(battTemp, 2);
    Serial.print(", ");
    SDout.print(", ");

    Serial.print(currentDraw, 2);
    SDout.print(currentDraw, 2);
